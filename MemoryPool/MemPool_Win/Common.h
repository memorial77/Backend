...
public:
	...
	FreeList() : list_(nullptr), size_(0), get_count_(1) {} // 构造函数
	...
	void pop_range(void*& start, void*& end, size_t n)
	{
		...
		list_ = next_obj(end);		// 更新list_指向
		next_obj(end) = nullptr;	// 将取出的内存块从链表中断开
		size_ -= n;					// 更新自由链表中内存块的个数
	}
...
private:
	void* list_;			// 自由链表头指针
	size_t size_;		// 自由链表中内存块的大小
	size_t get_count_; // 从中心缓存获取的内存块个数
};
...
struct Span
{
	...
	bool is_usud_ = false;		// 是否被使用
	size_t obj_size_ = 0;		// 内存块大小
};
