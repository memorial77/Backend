...
void ThreadCache::dealloc(void* ptr, size_t size)
{
	...
	// 如果自由链表中的内存块个数大于一定阈值，则将内存块释放到中心缓存
...
void ThreadCache::list_too_long_free(size_t index, size_t size)
{
	...
	size_t pop_size = thread_cache_free_list_[index].get_count();	// 获取的内存块个数
	thread_cache_free_list_[index].pop_range(start, end, pop_size); // 从自由链表中取出pop_size个内存块
